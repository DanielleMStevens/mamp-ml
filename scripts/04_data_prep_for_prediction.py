...
- input_excel.xlsx: Contains MAMP-receptor interactions of interest
...
def load_receptor_sequences(in_data_dir: Path) -> Dict[str, str]:
    fasta_path = in_data_dir / "intermediate_files" / "lrr_domain_sequences.fasta"
...
# Load Excel data
    excel_path = Path(sys.argv[1]) if len(sys.argv) > 1 else in_data_dir / "example_file.xlsx"
    if not excel_path.exists():
        raise FileNotFoundError(f"Could not find Excel data file at {excel_path}")
    data_df = pd.read_excel(excel_path, sheet_name='Sheet1')

    # Select required columns and create a copy
    required_columns = ["plant_species", "receptor", "locus_id", "receptor_sequence", "ligand_sequence"]
    ...
    original_receptor_names = receptor_ligand_pairs.apply(
        lambda x: f"{x['plant_species'].replace(' ', '_')}|{x['locus_id']}|{x['receptor']}",
        axis=1
    )
    ...
    cols = ['Header_Name'] + list(receptor_ligand_pairs.columns.drop('Header_Name'))
    receptor_ligand_pairs = receptor_ligand_pairs[cols]
    receptor_name_to_seq = load_receptor_sequences(in_data_dir)
    receptor_ligand_pairs['receptor_sequence'] = original_receptor_names.map(receptor_name_to_seq)
    receptor_ligand_pairs = receptor_ligand_pairs.dropna(subset=['receptor_sequence'])
    print(receptor_ligand_pairs)

    # Log missing sequences
    missing_sequences = receptor_ligand_pairs['receptor_sequence'].isna().sum()
    if missing_sequences > 0:
        ...
        missing_headers = receptor_ligand_pairs[receptor_ligand_pairs['receptor_sequence'].isna()]['Header_Name']
        for header in missing_headers:
            print(f"- {header}")

    # Handle legacy column names if needed
    if use_legacy_columns:
        column_mapping = {'ligand_sequence': 'Sequence'}
...
def main():
    ...
    in_data_dir = base_dir
    out_dir = base_dir / "intermediate_files"
